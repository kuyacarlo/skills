...
def test_run_cmd_success():
    # Verify run_cmd handles successful command execution
    res = gpg_verify.run_cmd(["echo", "hello"])
    assert res.returncode == 0
    assert res.stdout.strip() == "hello"


def test_run_cmd_file_not_found():
    # Verify run_cmd exits with code 2 when command executable is not found
    with patch("sys.exit") as mock_exit:
        gpg_verify.run_cmd(["non_existent_command_12345"])
        mock_exit.assert_called_once_with(2)


def test_get_commit_info_success():
    # Verify get_commit_info successfully parses and returns commit summary
    with patch("src.gpg_verify.run_cmd") as mock_run:
        mock_run.return_value = subprocess.CompletedProcess(
            args=["git", "log", "-1", "--format=%h - %an: %s", "commit123"],
            returncode=0,
            stdout="abc1234 - Alice: Initial commit\n",
            stderr="",
        )
        assert (
            gpg_verify.get_commit_info("commit123") == "abc1234 - Alice: Initial commit"
        )


def test_get_commit_info_failure():
    # Verify get_commit_info falls back to returning commit hash on git log failure
    with patch("src.gpg_verify.run_cmd") as mock_run:
        mock_run.return_value = subprocess.CompletedProcess(
            args=["git", "log", "-1", "--format=%h - %an: %s", "commit123"],
            returncode=128,
            stdout="",
            stderr="fatal: no such commit",
        )
        assert gpg_verify.get_commit_info("commit123") == "commit123"
