...
def get_commits_to_check(
    rev_range: Optional[str] = None, commits: Optional[List[str]] = None
) -> Optional[List[str]]:
    """Retrieves commit hashes to check.

    If `commits` is provided, those specific commits are returned.
    If `rev_range` is provided, the commits in that range are returned.
    If neither is provided, defaults to ["HEAD"].
    """
    if commits:
        resolved_commits = []
        for c in commits:
            res = run_cmd(["git", "rev-parse", "--verify", f"{c}^0"])
            if res.returncode != 0:
                print(f"Error: Invalid commit reference '{c}'", file=sys.stderr)
                return None
            resolved_commits.append(res.stdout.strip())
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify GPG signatures of git commits in a revision range or individual commits."
    )
    parser.add_argument(
        "--rev-range",
        type=str,
        help="Revision range to check (e.g. origin/main..HEAD). Defaults to HEAD if no commits specified.",
    )
    parser.add_argument(
        "--commits",
        type=str,
        nargs="+",
        help="Specific commit hashes to check (e.g. a1b2c3d e4f5g6h).",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if args.rev_range and args.commits:
        print("Error: Cannot specify both --rev-range and --commits.", file=sys.stderr)
        return 2

    commits = get_commits_to_check(args.rev_range, args.commits)
    if commits is None:
        return 2
