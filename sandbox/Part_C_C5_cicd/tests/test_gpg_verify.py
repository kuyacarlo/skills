...
def test_get_commits_to_check_commits_success():
    with patch("src.gpg_verify.run_cmd") as mock_run:
        mock_run.return_value = subprocess.CompletedProcess(
            args=["git", "rev-parse", "--verify", "commit1^0"],
            returncode=0,
            stdout="commit1_full_hash\n",
            stderr="",
        )
        assert gpg_verify.get_commits_to_check(commits=["commit1"]) == [
            "commit1_full_hash"
        ]


def test_get_commits_to_check_commits_failure():
    with patch("src.gpg_verify.run_cmd") as mock_run:
        mock_run.return_value = subprocess.CompletedProcess(
            args=["git", "rev-parse", "--verify", "invalid^0"],
            returncode=128,
            stdout="",
            stderr="fatal: Needed a single revision",
        )
        assert gpg_verify.get_commits_to_check(commits=["invalid"]) is None


@patch("src.gpg_verify.check_git_repo")
def test_main_both_range_and_commits(mock_check_repo):
    mock_check_repo.return_value = True
    with patch(
        "sys.argv",
        ["gpg_verify.py", "--rev-range", "HEAD~1..HEAD", "--commits", "commit1"],
    ):
        assert gpg_verify.main() == 2
...
@patch("src.gpg_verify.get_commits_to_check")
@patch("src.gpg_verify.verify_commit")
def test_main_commits_success(mock_verify, mock_get_commits, mock_check_repo):
    mock_check_repo.return_value = True
    mock_get_commits.return_value = ["commit1"]
    mock_verify.return_value = True

    with patch("sys.argv", ["gpg_verify.py", "--commits", "commit1"]):
        assert gpg_verify.main() == 0
        mock_get_commits.assert_called_once_with(None, ["commit1"])
