...
## 4. Feature Checklist
- [ ] **MVP (Must-Have)**
  - [ ] Implement `src/gpg_verify.py` core functionality to invoke `git verify-commit` on a range of commits.
  - [ ] Parse CLI arguments (`--rev-range`, `--verbose`).
  - [ ] Write unit tests for `gpg_verify.py` mocking `subprocess.run` to cover various git outputs and exit codes.
  - [ ] Configure `.pre-commit-config.yaml` to run `ruff check`, `ruff format`, unit tests, and GPG signature checks.
  - [ ] Create `.github/workflows/ci.yml` to lint, format, run tests, and check commit GPG signatures in GitHub Actions.

- [ ] **V1 (Should-Have)**
  - [ ] Automatic fallback to checking only commits in the current branch against target branch if running in GitHub Actions.
