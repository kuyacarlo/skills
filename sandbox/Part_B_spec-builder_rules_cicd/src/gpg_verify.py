...
def run_cmd(cmd: List[str]) -> subprocess.CompletedProcess[str]:
    """Runs shell commands safely and captures the outcome."""
    try:
...
def get_commit_info(commit: str) -> str:
    """Retrieves short metadata (hash, author, summary) of a commit for logging."""
...
def check_git_repo() -> bool:
    """Validates that the command is run inside a working Git repository tree."""
    res = run_cmd(["git", "rev-parse", "--is-inside-work-tree"])
    return res.returncode == 0


def get_commits_to_check(rev_range: Optional[str]) -> Optional[List[str]]:
    """Lists commit hashes in the specified range, defaulting to HEAD."""
    if not rev_range:
        return ["HEAD"]
...
def verify_commit(commit: str, verbose: bool) -> bool:
    """Checks the GPG signature validity of an individual commit."""
...
def main() -> int:
    """Main execution entry point parsing arguments and checking signatures."""
